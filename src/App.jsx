...
const EstimatorWizard = ({ user }) => {
  ...
  const calculateStackSpecific = (stackId) => {
    ...
    if (stackId === 'aws') {
      if (requirements.channels.includes('Voice')) {
        ...
        if (requirements.solutions.includes('qaAuto')) {
  ...
  const comparisonData = useMemo(() => {
    const vendors = ['aws', 'kore', 'yellow', 'five9', 'cresta', 'observe'];
    ...
    const results = vendors.map(id => {
      ...
      return { id, supported, costs };
    });

    const supportedCosts = results
      .filter(r => r.supported && r.costs)
      .map(r => r.costs.total);
    const minCost = supportedCosts.length > 0 ? Math.min(...supportedCosts) : 0;
    return results.map(r => ({
      ...r,
      isBest: r.supported && r.costs && r.costs.total === minCost,
      priceDiff:
        r.supported && r.costs && r.costs.total > minCost
          ? r.costs.total - minCost
          : 0
    }));
  }, [requirements, globalParams]);

  // --- ACTIONS ---

  const selectVendor = (stackId) => {
    setClient({ ...client, techStack: stackId });
    setStep(3);
  };
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {CAPABILITIES.solutions.map(sol => {
            ...
            return (
              <button key={sol.id} onClick={() => setRequirements({...requirements, solutions: active ? requirements.solutions.filter(s => s !== sol.id) : [...requirements.solutions, sol.id]})} className={`px-6 py-4 rounded-[16px] border-2 font-black transition-all flex items-center gap-4 ${active ? 'border-amber-500 bg-amber-50 text-amber-700 shadow-md scale-[1.01]' : 'border-slate-100 bg-white text-slate-500 hover:bg-slate-50'}`}>
    ...
    return (
      <div className="space-y-8 animate-fadeIn font-black">
        ...
        <div className="flex flex-col space-y-6 font-black font-black">
          {comparisonData.filter(v => v.supported).map(vendor => (
            <div key={vendor.id} className={`group bg-white border-4 rounded-[40px] p-6 lg:p-8 transition-all flex flex-col lg:flex-row items-stretch gap-10 relative overflow-hidden font-black ${vendor.isBest ? 'border-emerald-500 shadow-2xl shadow-emerald-500/10 font-black' : 'border-slate-50 hover:border-blue-400 shadow-xl shadow-slate-100'}`}>
              <div className="lg:w-[24%] flex flex-col justify-center items-center lg:items-start shrink-0 border-r border-slate-50 pr-8 font-black font-black">
                ...
                <div className="space-y-1 text-center lg:text-left font-black">
                  <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] font-black">Monthly Tech run cost</p>
                  <p className={`text-3xl font-black font-mono tracking-tighter ${vendor.isBest ? 'text-emerald-600' : 'text-slate-900'} font-black font-black`}>$ {vendor.costs.total.toLocaleString(undefined, {maximumFractionDigits:0})}</p>
                ...
                <button onClick={() => selectVendor(vendor.id)} className={`mt-10 w-full py-4 rounded-[20px] font-black uppercase text-[10px] tracking-[0.2em] flex items-center justify-center gap-3 transition-all active:scale-95 shadow-xl ${vendor.isBest ? 'bg-emerald-600 text-white hover:bg-emerald-700 shadow-emerald-500/20' : 'bg-slate-900 text-white hover:bg-blue-600 shadow-slate-900/20'}`}>Confirm <ArrowRight size={18}/></button>
              </div>
  ...
  const renderStep3 = () => {
    ...
    return (
      <div className="space-y-8 animate-fadeIn font-black">
        <div className="bg-slate-900 text-white rounded-3xl p-6 lg:p-8 shadow-lg flex items-center justify-between border-2 border-slate-800/80 relative overflow-hidden font-black">
          ...
          <div className="flex items-center gap-6 relative z-10 font-black">
            <div className="bg-white p-3 rounded-2xl border border-slate-700 shadow-md font-black font-black">
              <img
                src={`https://img.logo.dev/${client.techStack === 'aws' ? 'amazon.com' : client.techStack === 'five9' ? 'five9.com' : client.techStack + '.ai'}?token=${LOGO_TOKEN}`}
                className="h-8 w-auto font-black"
                alt="Selected Vendor"
              />
            </div>
            <div>
              <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1.5 font-black">
                Architecture baseline
              </p>
              <h2 className="text-2xl lg:text-3xl font-black uppercase tracking-tight font-black">
                {client.techStack} global deployment
              </h2>
            </div>
          </div>
          <div className="text-right relative z-10 font-black font-black font-black">
            <p className="text-[11px] font-black text-blue-300 uppercase tracking-widest mb-1.5 font-black">
              Tech OPEX
            </p>
            <p className="text-3xl lg:text-4xl font-black font-mono tracking-tighter text-white font-black font-black">
    ...
    <div className="max-w-[96%] mx-auto p-4 lg:p-10 min-h-screen flex flex-col font-black font-black">
      ...
      <div className="bg-white rounded-[80px] shadow-[0_60px_120px_-30px_rgba(0,0,0,0.06)] border border-slate-50 p-10 lg:p-16 min-h-[900px] flex flex-col transition-all duration-700 ease-in-out font-black font-black">
        ...
        <div className="mt-20 pt-16 flex justify-between items-center border-t border-slate-50 font-black font-black font-black font-black font-black font-black">
          {step === 1 && (
            <button onClick={() => setStep(2)} disabled={!client.name || requirements.channels.length === 0} className="ml-auto bg-blue-600 text-white px-16 py-6 rounded-[32px] font-black uppercase tracking-[0.4em] text-[12px] flex items-center gap-5 hover:bg-blue-700 disabled:opacity-10 transition-all shadow-2xl active:scale-95 font-black font-black font-black font-black font-black">Execute Analysis <ArrowRight size={22}/></button>
          )}
          {step > 1 && (
            <button onClick={() => setStep(step - 1)} className="flex items-center gap-5 text-slate-400 font-black hover:text-slate-900 transition-colors uppercase text-[12px] tracking-[0.4em] font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black"><ArrowLeft size={18}/> Return to Discovery</button>
          )}
          {step === 3 && (
            <button onClick={() => setStep(4)} disabled={resources.length === 0} className="bg-blue-600 text-white px-16 py-6 rounded-[32px] font-black uppercase tracking-[0.4em] text-[12px] flex items-center gap-5 hover:bg-blue-700 disabled:opacity-10 transition-all shadow-2xl active:scale-95 font-black font-black font-black font-black font-black font-black">Finalize Portfolio <ArrowRight size={22}/></button>
          )}
        </div>
      </div>
    </div>
